...
async function test() {
  ...
  console.log('Checking Diosmasgym...');
  const resD = await fetch(defaultDiosmasgymUrl);
  const csvD = await resD.text();
  const parsedD = parseMusicCSV(csvD);
  const configD = parsedD.filter(item => item.artist && item.artist.toUpperCase().includes('CONFIG'));
  console.log('Diosmasgym config items found:', configD);

  console.log('Checking Juan614...');
  const resJ = await fetch(defaultJuan614Url);
  const csvJ = await resJ.text();
  const parsedJ = parseMusicCSV(csvJ);
  const configJ = parsedJ.filter(item => item.artist && item.artist.toUpperCase().includes('CONFIG'));
  console.log('Juan614 config items found:', configJ);
}
