...
const SearchView: React.FC<SearchViewProps> = ({ catalog, onPlaySong }) => {
  ...
  const availableTypes = useMemo(() => {
    ...
    catalog.forEach(s => { if (s.type) types.add(s.type); });
  ...
  }, [catalog, query, artistFilter, typeFilter]);
        ...
        {results.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {results.map((song, idx) => (
              <div key={song.id} className="animate-fade-in-up" style={{ animationDelay: `${Math.min(idx * 40, 400)}ms` }}>
