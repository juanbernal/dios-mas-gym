...
// ── Helper: URL → dataURL through our same-origin proxy ────────────────────
...
const Top5SocialGenerator: React.FC = () => {
  ...
  // ── Key addition: pre-loaded dataURLs for every cover ──────────────────────
  ...
  // ── Pre-load covers as dataURLs whenever topSongs changes ─────────────────
  useEffect(() => {
    ...
    (async () => {
      ...
      await Promise.all(needed.map(async song => {
        const durl = await urlToDataUrl(song.cover);
        updates[song.id] = durl;
      }));
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (json?.data?.topSongs?.length > 0) {
          ...
          else alert("No se encontraron coincidencias en el catálogo.");
        } else { alert("No hay suficientes datos en las analíticas aún."); }
      }
    } catch (e) { console.error(e); alert("Error al obtener las analíticas reales."); }
    finally { setIsLoading(false); }
  };
  ...
  const handleSelectSong = (song: MusicItem) => {
    if (topSongs.length >= 5) { alert("Ya tienes 5 canciones. Elimina una primero."); return; }
    if (topSongs.find(s => s.id === song.id)) { alert("Esta canción ya está en el Top."); return; }
    setTopSongs([...topSongs, song]);
    setSearchQuery(""); setIsSearchOpen(false);
  };

  const handleRemoveSong = (id: string) => setTopSongs(topSongs.filter(s => s.id !== id));

  const moveSong = (index: number, dir: "up" | "down") => {
    const arr = [...topSongs];
  ...
  const handleDownload = async () => {
    ...
    if (!coversReady) { alert("Las imágenes aún se están cargando. Espera un momento."); return; }
    setIsGenerating(true);
    try {
      // Give the browser a moment to repaint with the dataURL sources
      await new Promise(r => setTimeout(r, 400));
      window.scrollTo(0, 0);

      const canvas = await html2canvas(captureRef.current, {
        scale: 3,           // → 3240 × 4050 px
        useCORS: false,     // all imgs are dataURLs — no CORS needed
        allowTaint: true,
  ...
  // Cover helper: returns pre-loaded dataURL or empty string
  const cover = (song: MusicItem) => coverDataUrls[song.id] || "";
    ...
    <div className="flex flex-col bg-[#05070a] min-h-screen text-white font-['Poppins'] pb-20">
      ...
      <div className="sticky top-0 z-[100] bg-black/80 backdrop-blur-xl border-b border-white/5 p-4 flex items-center justify-between">
        <button onClick={() => navigate("/admin")}
          ...
          <i className="fas fa-chevron-left text-[8px]" /> Volver al Dashboard
        </button>
      ...
      <div className="flex-1 p-6 md:p-10 max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="bg-[#0f111a] border border-white/5 rounded-3xl p-8 shadow-2xl">
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                <label className="text-[9px] uppercase font-bold text-white/30 tracking-widest">Subtítulo</label>
              ...
              <button onClick={handleAutoFill} disabled={isLoading || catalog.length === 0}
                ...
                <i className={`fas ${isLoading ? "fa-spinner fa-spin" : "fa-magic"}`} /> Autocompletar con Analíticas Reales
              </button>
              ...
              <div className="relative">
                ...
                <div className="absolute inset-y-0 left-4 top-6 flex items-center pointer-events-none text-[#c5a059]/40 mt-1"><i className="fas fa-search text-xs" /></div>
                ...
                {isSearchOpen && (
                  <div className="absolute top-full left-0 right-0 mt-2 bg-[#0a0f1d] border border-white/10 rounded-2xl overflow-hidden z-[200] shadow-2xl max-h-60 overflow-y-auto">
                    {filteredCatalog.length > 0 ? filteredCatalog.map(song => (
                      <button key={song.id} onClick={() => handleSelectSong(song)}
                        ...
                        {/* Use proxy URL for preview list (CORS not needed here) */}
                  ...
                  <div key={song.id} className="flex items-center gap-3 bg-black/30 border border-white/5 p-3 rounded-xl">
                    ...
                    <div className="w-10 h-10 rounded overflow-hidden bg-black shrink-0">
                      {coverDataUrls[song.id]
                        ? <img src={coverDataUrls[song.id]} alt="" className="w-full h-full object-cover" />
                        : <div className="w-full h-full flex items-center justify-center"><i className="fas fa-spinner fa-spin text-[#c5a059]/40 text-xs" /></div>}
                    </div>
                    <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-col gap-1">
                      <button onClick={() => moveSong(index, "up")} disabled={index === 0} className="text-white/30 hover:text-white disabled:opacity-30"><i className="fas fa-chevron-up text-[10px]" /></button>
                      <button onClick={() => moveSong(index, "down")} disabled={index === topSongs.length - 1} className="text-white/30 hover:text-white disabled:opacity-30"><i className="fas fa-chevron-down text-[10px]" /></button>
                    </div>
              ...
              {/* Status indicator */}
              {topSongs.length > 0 && (
                <div className={`flex items-center gap-2 text-[9px] font-black uppercase tracking-widest ${coversReady ? "text-green-400" : "text-[#c5a059]"}`}>
                  <i className={`fas ${coversReady ? "fa-check-circle" : "fa-spinner fa-spin"}`} />
                  {coversReady ? "Imágenes listas para exportar" : "Cargando imágenes para exportación..."}
                </div>
              )}

              <button onClick={handleDownload} disabled={isLoading || isGenerating || topSongs.length === 0 || !coversReady}
                ...
                {isGenerating ? <><i className="fas fa-spinner fa-spin" /> Generando...</> : <><i className="fas fa-download" /> Descargar Imagen HD (3240×4050)</>}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 flex justify-center items-start overflow-hidden">
          <div style={{ width:"1080px", transformOrigin:"top center" }} className="scale-[0.45] md:scale-[0.55] lg:scale-[0.6] origin-top">
            <div ref={captureRef}
              className="relative bg-[#020202] flex flex-col items-center justify-between py-12 px-16 overflow-hidden"
              style={{ width:"1080px", height:"1350px" }}>

              {/* ── Background: <img> tag (not CSS bg-image) so html2canvas captures it ── */}
              {topSongs[0] && cover(topSongs[0]) && (
                <img src={cover(topSongs[0])} alt=""
                  style={{ position:"absolute", inset:0, width:"100%", height:"100%", objectFit:"cover", opacity:0.2, filter:"blur(30px)", transform:"scale(1.1)", pointerEvents:"none" }} />
              )}
              ...
              <div className="relative z-10 w-full flex flex-col items-center flex-1">
                ...
                <div className="text-center mb-10">
                  ...
                  {subtitle && (
                    <p className="text-4xl font-black uppercase tracking-[0.4em] text-white/70 mt-6 bg-[#c5a059]/10 py-3 px-8 rounded-full border border-[#c5a059]/30 inline-block">
                      {subtitle}
                    </p>
                  )}
                </div>
                ...
                <div className="w-full max-w-4xl space-y-4 flex-1 flex flex-col justify-center mb-6">
                  {topSongs.map((song, index) => (
                    <div key={song.id} className="flex items-center gap-8 bg-gradient-to-r from-white/10 to-transparent backdrop-blur-md p-5 rounded-3xl border-l-4 border-[#c5a059] shadow-2xl relative overflow-hidden">
                      ...
                      <div className="text-6xl font-serif italic text-[#c5a059] w-16 text-center drop-shadow-lg shrink-0">{index + 1}</div>
                      <div className="w-28 h-28 shrink-0 rounded-2xl shadow-[0_0_20px_rgba(0,0,0,0.5)] z-10 overflow-hidden bg-black flex items-center justify-center">
                        {cover(song)
                          ? <img src={cover(song)} className="w-full h-full object-cover" alt={song.name} />
                ...
                <div className="w-full mt-auto flex flex-col items-center pb-6">
                  ...
                  <div className="flex items-center justify-between w-full max-w-4xl px-8">
                    <div className="flex items-center gap-5 opacity-90">
                      <img src="/logo-diosmasgym.png" className="w-[70px] h-[70px] grayscale brightness-200 shrink-0" alt="Logo" />
                      <span className="text-[26px] font-black uppercase tracking-[0.4em] text-white">DIOSMASGYM</span>
                    </div>
