...
const LyricsManager: React.FC = () => {
    ...
    const handleBulkImport = async () => {
        ...
        try {
            ...
            for (const p of parsed) {
                const existingIdx = local.findIndex((l: any) => (l.name || '').trim().toLowerCase() === p.title.trim().toLowerCase() && (l.artist || '').trim().toLowerCase() === p.artist.trim().toLowerCase());
                const draftData = { name: p.title, artist: p.artist, content: p.content, sync: '', date: getFirstDate(p.title, p.artist, now) };
                if (existingIdx >= 0) { local[existingIdx] = draftData; updated++; }
                else { local.unshift(draftData); added++; }
            }
            localStorage.setItem('lyric_studio_drafts', JSON.stringify(local));
            await recordVersions(parsed.map(p => ({ title: p.title, artist: p.artist, content: p.content, date: now })));
            showNotification(`✅ Importadas ${added} letras nuevas${updated > 0 ? ` (${updated} actualizadas)` : ''} como Borradores Locales`);
            setBulkImportText('');
            setShowBulkImportModal(false);
            await loadAllLyrics();
        } catch (e: any) {
            showNotification('❌ Error al importar: ' + (e?.message || e));
        } finally {
    ...
    const extractYoutubeId = (url: string): string => {
        const m = (url || '').match(/(?:v=|youtu\.be\/|youtube\.com\/shorts\/|youtube\.com\/embed\/)([\w-]{11})/);
        return m ? m[1] : '';
    };

    // Limpieza ligera de la descripcion de YouTube: quita lineas que casi seguro NO son letra
    // (links, redes sociales, creditos de distribuidora). No es perfecta a proposito: se deja
    // todo en el textarea de importacion masiva para que el usuario la revise antes de guardar.
    const stripNonLyricNoise = (desc: string): string => {
        const noisePatterns = [
            /^https?:\/\//i, /^www\./i, /suscr[ií]bete/i, /s[ií]guenos en/i, /sigue a/i, /follow (me|us)/i,
            /instagram|tiktok|facebook\.com|spotify|apple music|deezer|tidal|amazon music|youtube\.com|youtu\.be/i,
            /^#\w+/, /provided to youtube by/i, /auto-generated by youtube/i, /℗/, /^copyright/i,
            /^letra\s*:?\s*$/i, /^lyrics\s*:?\s*$/i,
        ];
        return desc
            .split('\n')
            .filter(line => !noisePatterns.some(re => re.test(line.trim())))
            .join('\n')
            .replace(/\n{3,}/g, '\n\n')
            .trim();
    };

    // Trae la descripcion de YouTube de las canciones del catalogo que aun no tienen letra guardada
    // (muchos artistas pegan la letra completa ahi al subir el video), y la deja lista para revisar
    // en el modal de Importacion Masiva en vez de tener que copiarla una por una.
    const handleFetchFromYoutube = async () => {
        if (fullCatalog.length === 0) { showNotification('Espera a que cargue el catálogo primero.'); return; }
        const existingKeys = new Set(
            lyrics.filter(l => stripHtml(l.content).trim().length > 0)
                .map(l => `${l.artist.trim().toLowerCase()}|${l.title.trim().toLowerCase()}`)
        );
        const idToSong = new Map<string, MusicItem>();
        fullCatalog.forEach(s => {
            const key = `${(s.artist || '').trim().toLowerCase()}|${(s.name || '').trim().toLowerCase()}`;
            if (existingKeys.has(key)) return;
            if (s.lyrics && s.lyrics.trim().length > 0) return;
            const id = extractYoutubeId(s.url || '');
            if (id && !idToSong.has(id)) idToSong.set(id, s);
        });
        if (idToSong.size === 0) { showNotification('No hay canciones sin letra con video de YouTube para traer.'); return; }
        setIsFetchingYoutubeDescriptions(true);
        try {
            const ids = [...idToSong.keys()];
            const adminPass = localStorage.getItem('admin_password') || '';
            const res = await fetch(`/api/common?action=youtube-descriptions&ids=${ids.join(',')}`, {
                headers: { 'x-admin-password': adminPass }
            });
            const json = await res.json();
            if (!json.success) { showNotification('❌ ' + (json.message || 'No se pudo consultar YouTube')); return; }
            const blocks: string[] = [];
            idToSong.forEach((song, id) => {
                const entry = json.descriptions?.[id];
                if (!entry?.description) return;
                const cleaned = stripNonLyricNoise(entry.description);
                if (cleaned.trim().length < 20) return; // muy corto para ser letra
                const artist = song.artist === 'Juan 614' ? 'Juan 614' : 'Diosmasgym';
                blocks.push(`${song.name} - ${artist}\n${cleaned}`);
            });
            if (blocks.length === 0) { showNotification('No se encontró texto útil en las descripciones de esos videos.'); return; }
            setBulkImportText(blocks.join('\n\n---\n\n'));
            setShowBulkImportModal(true);
            showNotification(`✅ Se trajeron ${blocks.length} descripciones de YouTube. Revísalas: pueden traer texto de más (créditos, links, etc).`);
        } catch (e: any) {
            showNotification('❌ Error: ' + e.message);
        } finally {
            setIsFetchingYoutubeDescriptions(false);
        }
    };
    ...
    return (
                    ...
                    <div className="flex flex-wrap items-center gap-2 md:gap-4">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button
                                ...
                                disabled={isFetchingYoutubeDescriptions || loading}
                                className={`p-2.5 md:px-4 md:py-2 bg-red-500/10 border border-red-500/30 text-red-300 rounded-full transition-all flex items-center gap-2 ${isFetchingYoutubeDescriptions ? 'opacity-50' : ''}`}
                                title="Trae las descripciones de YouTube de tus canciones sin letra (muchos artistas la pegan ahí al subir el video)"
                            >
                                <i className={`fab fa-youtube ${isFetchingYoutubeDescriptions ? 'fa-spin' : ''}`}></i>
                            ...
                            >
                                ...
                                <span className="hidden md:inline text-[9px] font-black uppercase tracking-widest">Importar Muchas</span>
                            </button>
                            <button
                                onClick={() => {
                                    setNewLyricTitle('');
        ...
        {showBulkImportModal && (
            <div className="fixed inset-0 z-[1000] bg-black/90 backdrop-blur-xl flex items-center justify-center p-4">
                <div className="bg-[#0a0c14] border border-white/10 rounded-3xl w-full max-w-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col max-h-[90vh]">
                    <div className="p-6 border-b border-white/5 flex items-center gap-4 bg-gradient-to-r from-indigo-500/10 to-transparent">
                        <div className="w-12 h-12 bg-indigo-500/10 rounded-2xl flex items-center justify-center text-indigo-300 shadow-[0_0_20px_rgba(99,102,241,0.15)]">
                            <i className="fas fa-layer-group text-xl"></i>
                        </div>
                        <div>
                            <h3 className="text-xl font-black uppercase tracking-tight text-white">Importación <span className="text-indigo-300">Masiva</span></h3>
                            <p className="text-[9px] uppercase font-bold tracking-[0.2em] text-white/40">Pega todas tus letras juntas y se crean solas</p>
                        </div>
                    </div>
                    <div className="p-6 overflow-y-auto space-y-4">
                        ...
                        <textarea
                            ...
                            placeholder="Pega aquí todas tus letras juntas, separadas por una línea con ---"
                            rows={12}
                            className="w-full bg-black/50 border border-white/10 rounded-2xl p-4 text-white text-xs leading-relaxed outline-none focus:border-indigo-400/50 focus:ring-1 focus:ring-indigo-400/30 transition-all font-mono resize-y"
                        />
