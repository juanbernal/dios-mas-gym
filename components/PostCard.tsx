...
const PostCard: React.FC<PostCardProps> = ({ post, onClick, isFav, isRead, onFav, size = 'md' }) => {
  ...
  return (
    <div 
      className={`group relative flex flex-col h-full bg-[#0f111a]/90 border border-white/5 rounded-[1.75rem] overflow-hidden cursor-pointer hover:shadow-[0_35px_90px_-25px_rgba(197,160,89,0.28)] hover:border-[#c5a059]/25 transition-all duration-500 gold-border-glow ${isSm ? 'min-h-[320px]' : ''}`}
      onClick={onClick}
    >
      <div className={`relative ${imgHeight} overflow-hidden`}>
        ...
        <div className="absolute left-6 bottom-6 px-3 py-1.5 rounded-full bg-black/40 border border-white/10 backdrop-blur-xl text-[8px] font-black uppercase tracking-[0.35em] text-[#c5a059] flex items-center gap-2">
          <span>Lectura</span>
        ...
        {isRead && (
          <div className="absolute top-6 left-6 px-3 py-1.5 rounded-full bg-[#c5a059]/20 border border-[#c5a059]/30 backdrop-blur-xl text-[7px] font-black uppercase tracking-[0.2em] text-[#c5a059]">
      ...
      <div className={`${padding} flex-1 flex flex-col relative z-10 -mt-1`}>
        <div className="flex gap-4 mb-4 flex-wrap">
           {post.labels?.slice(0, 2).map(l => (
              <span key={l} className={`${badgeSize} font-black uppercase tracking-[0.5em] text-[#c5a059]`}>
                {l}
              </span>
           ))}
        </div>
        
        <h4 className={`font-serif font-bold mb-4 leading-[1.12] text-white/95 group-hover:text-[#c5a059] transition-colors ${titleSize}`}>
          {post.title}
        </h4>
