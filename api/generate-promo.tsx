...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (!BOT_TOKEN || !CHAT_ID) {
      return res.status(400).json({ error: "TELEGRAM_BOT_TOKEN or TELEGRAM_CHAT_ID not found." });
    }
    ...
    return res.status(200).json({ 
      success: true, 
      final_mode: "restored-design-nodejs",
      song: song.name 
    });

  } catch (error: any) {
    return res.status(500).json({ error_crash: error.message });
  }
}
